...
class Client:
    ...
    async def connect(self, cluster_id, client_id,
        ...
        if resp.protocol >= PROTOCOL_ONE:
            if resp.pingInterval != 0:
                ...
                self._ping_server_task = asyncio.get_running_loop().create_task(
                    self._ping_server())
    ...
    async def publish(self, subject, payload,
        ...
        if ack_handler is not None:
            ...
            try:
                ...
                await asyncio.wait_for(future, ack_wait)
    ...
    async def subscribe(self, subject,
        ...
        sub._msgs_task = asyncio.get_running_loop().create_task(self._process_msg(sub))
    ...
    async def _close(self):
        ...
        try:
            ...
            if self._hb_inbox_sid is not None:
                await self._hb_inbox_sid.unsubscribe()
                self._hb_inbox = None
                self._hb_inbox_sid = None
            if self._ack_subject_sid is not None:
                await self._ack_subject_sid.unsubscribe()
                self._ack_subject = None
                self._ack_subject_sid = None
        except:
